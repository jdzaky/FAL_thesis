...
def calculate_fairness_metrics(df, label_col='best_controller_label'):
    ...
    privileged = df[df[FAIRNESS_CONFIG['sensitive_attr']].isin(FAIRNESS_CONFIG['privileged_groups'])]
    disadvantaged = df[df[FAIRNESS_CONFIG['sensitive_attr']].isin(FAIRNESS_CONFIG['disadvantaged_groups'])]
    
    priv_best = privileged.groupby(label_col)['ahp_score'].mean()
    dis_best = disadvantaged.groupby(label_col)['ahp_score'].mean()
    
    metrics = {
        'privileged_mean_score': privileged['ahp_score'].mean(),
        'disadvantaged_mean_score': disadvantaged['ahp_score'].mean(),
        'score_gap': privileged['ahp_score'].mean() - disadvantaged['ahp_score'].mean(),
        'privileged_best_controller': priv_best.idxmax(),
        'disadvantaged_best_controller': dis_best.idxmax(),
        'statistical_parity_difference': abs(priv_best.max() - dis_best.max())
    }
    
    logger.info(f"Privileged mean: {metrics['privileged_mean_score']:.4f}")
    logger.info(f"Disadvantaged mean: {metrics['disadvantaged_mean_score']:.4f}")
    logger.info(f"Score gap: {metrics['score_gap']:.4f}")
    logger.info(f"SPD: {metrics['statistical_parity_difference']:.4f}")
