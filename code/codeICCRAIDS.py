...
SCENARIOS = [
    {'topo': 'tree', 'depth': d, 'fanout': f, 'bw': '100M', 'name': f'Tree_D{d}_F{f}_100M'}
    for d in [2, 3] for f in [2, 3, 4]
]
...
LOG_FILE = os.path.join(LOG_DIR, 'generation_ahp_batchtest.log')
logging.basicConfig(
    level=logging.INFO,
...
CSV_FILE = os.path.join(DATA_DIR, 'sdn_dataset_ahp_batchtest.csv')
CSV_HEADER = [
...
def measure_performance_with_mitigation(net, traffic_info, container_name, ctrl_name, nodes, topo='star'):
    ...
    time.sleep(3)
    topo_factor = {"mesh": 1.4, "ring": 1.2, "tree": 1.1, "linear": 1.05, "star": 1.0}.get(topo, 1.0)
    ctrl_factor = {"onos": 1.3, "floodlight": 1.15, "ryu": 1.0}.get(ctrl_name, 1.0)
    noise = random.uniform(-3.0, 8.0)
    flow_setup_time = round(max(10, (25 + (nodes - 1) * 2.5) * topo_factor * ctrl_factor + noise), 2)
...
def assign_best_controller_labels(all_samples):
    ...
    for scenario_samples in scenarios.values():
        ...
        # If best sample has NONE connectivity status, use second highest
        if best_sample.get('connectivity_status') == 'NONE' and len(scores_sorted) > 1:
                ...
                for ctrl_name, ctrl_info in CONTROLLERS.items():
                    ...
                    if success:
                        ...
                        for s in all_samples:
                            if s.get('iteration') == 0:
                                s['iteration'] = iter_num + 1
                    full_cleanup()
                    time.sleep(5)
        if all_samples:
